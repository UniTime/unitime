...
<tt:back-mark back="true" clear="true" title="${MSG.backNotAssignedExaminations()}" uri="unassignedExams.action"/>
<s:form action="unassignedExams">
		...
		<table class="unitime-MainTable">
			<tr><td><i>
				...
				<s:if test="form.subjectArea < 0">
					<loc:message name="messageAllExamsAreAssinged"/>
				</s:if>
				<s:if test="form.subjectArea > 0">
					<loc:message name="messageAllExamsOfASubjectAreAssinged"><s:property value="form.subjectAreaAbbv"/></loc:message>
				</s:if>
			</i></td></tr>
		</table>
	</s:if>
