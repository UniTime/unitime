...
alter table degree add constraint pk_degree primary key (uniqueid);
create unique index uk_degree on degree using btree (session_id, reference);

alter table degree add constraint fk_degree_session foreign key (session_id)
	references sessions (uniqueid) on delete cascade;

alter table student_area_clasf_major add degree_id bigint;
