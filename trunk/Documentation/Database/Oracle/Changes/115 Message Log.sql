...
insert into saved_hql (uniqueid, name, description, query, type) values
	(pref_group_seq.nextval, 'Message Log', 'Display message log', 'select timeStamp as Time, (case level when 50000 then ''<font color="red">Fatal</font>'' when 40000 then ''<font color="red">Error</font>'' when 30000 then ''<font color="orange">Warning</font>'' when 20000 then ''Info'' when 10000 then ''Debug'' else ''Other'' end) as Level, logger as Logger, (case when exception is null then message when message is null then exception else (message || ''\\n'' || exception) end) as Message, (case when ndc is null then thread else (thread || ''\\n'' || ndc) end) as Context from MessageLog order by timeStamp desc', 16);
