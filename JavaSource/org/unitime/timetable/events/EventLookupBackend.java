			...
			try {
				...
				case CURRICULUM:
					curriculum = hibSession.createQuery(
							...
							.setParameter("resourceId", request.getResourceId()).setCacheable(true).uniqueResult();
					if (!curriculum.isMultipleMajors() || curriculum.getMajors().isEmpty()) {
						curriculumCourses = hibSession.createQuery(
								...
								.setParameter("resourceId", request.getResourceId()).list();
						...
						for (Object[] o: hibSession.createQuery(
								...
								Object[].class)
								.setParameter("resourceId", request.getResourceId()).setCacheable(true).list()) {
						...
						List<Curriculum> parents = hibSession.createQuery(
								...
								Curriculum.class)
								.setParameter("curriculumId", curriculum.getUniqueId()).setCacheable(true).list();
						for (CurriculumClassification clasf: curriculum.getClassifications()) {
							...
							for (Curriculum parent: parents) {
								...
								for (CurriculumClassification parentClasf: parent.getClassifications()) {
									if (!clasf.getAcademicClassification().equals(parentClasf.getAcademicClassification())) continue;
									for (CurriculumCourse cc: parentClasf.getCourses()) {
										if (cc.getPercShare() > 0.0f) courses.add(cc.getCourse().getUniqueId());
									}
								}
							}
							for (CurriculumCourse cc: clasf.getCourses()) {
								if (cc.getPercShare() > 0.0f) courses.add(cc.getCourse().getUniqueId());
								else courses.remove(cc.getCourse().getUniqueId());
							}
						...
						if (limit <= 0 || meetings.size() < limit)
							meetings.addAll(query.select("distinct m").type("ClassEvent")
									.from("inner join e.clazz.schedulingSubpart.instrOfferingConfig.instructionalOffering.courseOfferings co")
									.where("co.uniqueId in (:courses)")
									.set("courses", curriculumCourses)
									.limit(limit <= 0 ? -1 : 1 + limit - meetings.size())
									.query(hibSession).list());
						...
						for (Object[] o: hibSession.createQuery(
								...
								Object[].class)
								.setParameter("resourceId", request.getResourceId()).setParameterList("courses", curriculumCourses).setCacheable(true).list()) {
							Long offeringId = (Long)o[0];
							Long configId = (Long)o[1];
							Long clazzId = (Long)o[2];
							Set<Long>[] r = restrictions.get(offeringId);
						...
						if (limit <= 0 || meetings.size() < limit)
							meetings.addAll(query.select("distinct m").type("ExamEvent")
								.from("inner join e.exam.owners o")
								.where("o.ownerType = :type and o.ownerId in (:courses)")
								.set("courses", curriculumCourses)
								.set("type", ExamOwner.sOwnerTypeCourse)
								.limit(limit <= 0 ? -1 : 1 + limit - meetings.size())
								.query(hibSession).list());
				...
				case PERSON:
					...
					boolean sessionCheck = !allSessions || !ApplicationProperty.EventsPersonalScheduleSkipSessionRestriction.isTrue();
					Set<String> roles = request.getEventFilter().getOptions("role");
					...
					if (allSessions && student) {
						for (Student s: hibSession.createQuery("from Student where externalUniqueId = :externalId", Student.class)
								.setParameter("externalId", request.getResourceExternalId()).setCacheable(true).list()) {
							...
							if (status != null && status.hasOption(StudentSectioningStatus.Option.noschedule)) {
								if (excludeSessionIds == null) excludeSessionIds = new HashSet<Long>();
					...
					if (allSessions) {
						if (student) {
							curriculumCourses.addAll(hibSession.createQuery("select e.courseOffering.uniqueId from StudentClassEnrollment e where e.student.externalUniqueId = :externalId", Long.class)
								.setParameter("externalId", request.getResourceExternalId()).list());
						...
						if (student) {
							curriculumCourses.addAll(hibSession.createQuery("select e.courseOffering.uniqueId from StudentClassEnrollment e where e.student.session.uniqueId = :sessionId and e.student.externalUniqueId = :externalId", Long.class)
									.setParameter("sessionId", request.getSessionId()).setParameter("externalId", request.getResourceExternalId()).list());
					...
					if (allSessions) {
						if (student && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query.select("distinct m").type("ClassEvent").from("inner join e.clazz.studentEnrollments enrl")
									.where("enrl.student.externalUniqueId = :externalId").where(sessionCheck, "enrl.student.session.uniqueId = s.uniqueId")
									.set("externalId", request.getResourceExternalId())
									.where("enrl.student.session.uniqueId not in :excludeSessionIds", "excludeSessionIds", excludeSessionIds)
						...
						if (instructor && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query.select("distinct m").type("ClassEvent").from("inner join e.clazz.classInstructors ci")
									.where("ci.instructor.externalUniqueId = :externalId").where(sessionCheck, "ci.instructor.department.session.uniqueId = s.uniqueId")
						...
						if (coordinator && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query.select("distinct m").type("ClassEvent").from("inner join e.clazz.schedulingSubpart.instrOfferingConfig.instructionalOffering.offeringCoordinators cc")
									.where("cc.instructor.externalUniqueId = :externalId").where(sessionCheck, "cc.instructor.department.session.uniqueId = s.uniqueId")
									.set("externalId", request.getResourceExternalId()).limit(limit <= 0 ? -1 : 1 + limit - meetings.size()).query(hibSession).list());
					} else {
						if (student && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query.select("distinct m").type("ClassEvent").from("inner join e.clazz.studentEnrollments enrl")
					...
					if (allSessions) {
						if (student && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query
									...
									.where("o.ownerType = :type and o.ownerId = co.uniqueId")
									.set("type", ExamOwner.sOwnerTypeCourse)
									.where(sessionCheck, "enrl.student.session.uniqueId = s.uniqueId")
									.where("enrl.student.session.uniqueId not in :excludeSessionIds", "excludeSessionIds", excludeSessionIds)
									.limit(limit <= 0 ? -1 : 1 + limit - meetings.size()).query(hibSession).list());
						if (student && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query
						...
						if (instructor && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query
									...
									.set("externalId", request.getResourceExternalId())
									.where(sessionCheck, "i.department.session.uniqueId = s.uniqueId")
									.limit(limit <= 0 ? -1 : 1 + limit - meetings.size()).query(hibSession).list());
						if (instructor && ApplicationProperty.EventExamsConsiderClassInstructorAssignments.isTrue() && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query
									...
									.where("o.ownerType = :type and o.ownerId = co.uniqueId")
									.set("type", ExamOwner.sOwnerTypeCourse)
									.where(sessionCheck, "ci.instructor.department.session.uniqueId = s.uniqueId")
									.limit(limit <= 0 ? -1 : 1 + limit - meetings.size()).query(hibSession).list());
						if (instructor && ApplicationProperty.EventExamsConsiderClassInstructorAssignments.isTrue() && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query
									.select("distinct m")
						...
						if (coordinator && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query
									...
									.where("o.ownerType = :type and o.ownerId = co.uniqueId")
									.set("type", ExamOwner.sOwnerTypeCourse)
									.where(sessionCheck, "cc.instructor.department.session.uniqueId = s.uniqueId")
									.limit(limit <= 0 ? -1 : 1 + limit - meetings.size()).query(hibSession).list());
						if (coordinator && (limit <= 0 || meetings.size() < limit))
							meetings.addAll(query
									.select("distinct m")
					...
					if (contact && (limit <= 0 || meetings.size() < limit))
						meetings.addAll(query.select("distinct m")
                    		.where("type(e) in (CourseEvent, SpecialEvent, UnavailableEvent)")
					...
					if (allSessions) {
						if (group.getExternalUniqueId() != null) {
							curriculumCourses.addAll(hibSession.createQuery("select distinct e.courseOffering.uniqueId from StudentGroup g inner join g.students s inner join s.classEnrollments e where g.externalId = :externalId", Long.class)
									.setParameter("externalId", group.getExternalUniqueId()).list());
							...
							curriculumCourses.addAll(hibSession.createQuery("select distinct e.courseOffering.uniqueId from StudentGroup g inner join g.students s inner join s.classEnrollments e where g.groupAbbreviation = :abbreviation", Long.class)
									.setParameter("abbreviation", group.getGroupAbbreviation()).list());
						...
						curriculumCourses.addAll(hibSession.createQuery("select distinct e.courseOffering.uniqueId from StudentGroup g inner join g.students s inner join s.classEnrollments e where g.uniqueId = :resourceId", Long.class)
								.setParameter("resourceId", group.getUniqueId()).list());
				...
				if (request.getEventFilter().hasOptions("flag") && request.getEventFilter().getOptions("flag").contains("Conflicts")) {
					...
					case CURRICULUM:
						...
						if (curriculum != null && curriculum.isMultipleMajors() && !curriculum.getMajors().isEmpty()) {
							if (curriculumCourses.isEmpty()) break;
							conflicts.addAll(query.select("distinct m.event.uniqueId, Xm").type("ClassEvent")
									.from("inner join e.clazz.schedulingSubpart.instrOfferingConfig.instructionalOffering.courseOfferings co")
									.where("co.uniqueId in (:courses)")
							...
							conflicts.addAll(query.select("distinct m.event.uniqueId, Xm").type("ExamEvent")
									...
									.query(hibSession).list());
							conflicts.addAll(query.select("distinct m.event.uniqueId, Xm").type("CourseEvent")
									.from("inner join e.relatedCourses o")
									.where("o.ownerType = :type and o.ownerId in (:courses)")
									.set("courses", curriculumCourses)
									.set("type", ExamOwner.sOwnerTypeCourse)
									.query(hibSession).list());
				...
				if ((!request.getEventFilter().hasOptions("type") || request.getEventFilter().getOptions("type").contains("Class")) &&
					...
					case COURSE:
						arrageHourClasses = hibSession.createQuery(
								...
								.setParameter("resourceId", request.getResourceId())
					...
					case PERSON:
						...
						if (student)
							arrageHourClasses.addAll(hibSession.createQuery(
								...
								Class_.class)
								.setParameter("externalId", request.getResourceExternalId()).setParameter("sessionId", request.getSessionId())
								.setCacheable(true).list());
					...
					case GROUP:
						...
						if (minEnrollment == null)
							arrageHourClasses.addAll(hibSession.createQuery(
								...
								Class_.class)
								.setParameter("resourceId", group.getUniqueId())
								.setCacheable(true).list());
						else
							arrageHourClasses.addAll(hibSession.createQuery(
									"select distinct c from StudentGroup g inner join g.students s inner join s.classEnrollments e inner join e.clazz c where c.committedAssignment is null and c.cancelled = false and g.uniqueId = :resourceId "+
									...
									Class_.class)
									.setParameter("resourceId", group.getUniqueId())
									.setParameter("minEnrl", minEnrollment)
									.setCacheable(true).list());
						break;
					}
