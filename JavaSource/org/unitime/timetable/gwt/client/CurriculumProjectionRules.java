...
public class CurriculumProjectionRules extends Composite {
	...
	public CurriculumProjectionRules() {
		...
		HorizontalPanel[] hp = new HorizontalPanel[] { new HorizontalPanel(), new HorizontalPanel()};
		hp[0].setStyleName("unitime-MainTableHeader");
		...
		for (int i = 0; i < 2; i++) {
			...
			hp[i].add(iErrorMessage[i]);
			hp[i].add(iSave[i]);
			hp[i].setCellWidth(iSave[i], "1%");
		}
	...
	public void showError(String error) {
		for (int i = 0; i < iErrorMessage.length; i++) {
			...
			iErrorMessage[i].setVisible(i == 0 || iTable.isVisible());
		}
	}
	...
	public void refreshTable() {
		...
		if (iRules == null || iRules.isEmpty()) {
			showError("No academic areas defined.");
			return;
		}
		...
		for (AcademicAreaInterface area: areas) {
			...
			MyRow r = new MyRow(area, null, rules.get(defaultMajor));
			if (classifications == null) {
				classifications = r.getClassifications();
			...
			for (MajorInterface major: new TreeSet<MajorInterface>(iRules.get(area).keySet())) {
				...
				r = new MyRow(area, major, rules.get(major));
				if (!r.hasLastLike()) continue;
		...
		if (classifications == null || classifications.isEmpty()) {
			showError("No academic classifications defined.");
			return;
		}
		...
		if (row == 1) {
			showError("No last-like enrollments.");
			return;
		}
	...
	private class MyCell extends Composite {
				...
				public void onChange(ChangeEvent event) {
					try {
						if (iTextBox.getText().isEmpty()) {
							iRow.setProjection(iClasf, 1.0f);
		...
		public void update() {
			...
			iTextBox.getElement().getStyle().setColor(projection == 1.0f ? "#777777" : null);
	...
	private class MyRow {
		...
		public float getProjection(AcademicClassificationInterface clasf) {
			return iData.get(clasf)[0].floatValue();
		}
		...
		public boolean hasProjection() {
			for (Number[] n: iData.values()) {
				if (n[1].intValue() > 0 && n[0].floatValue() != 1.0f) return true;
			}
			return false;
		}
	}
