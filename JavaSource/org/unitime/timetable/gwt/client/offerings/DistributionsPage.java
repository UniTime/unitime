...
public class DistributionsPage extends Composite {
	...
	public DistributionsPage() {
		...
		iFilter.getHeader().setCollapsible(!"0".equals(ToolBox.getSessionCookie("Distributions.Filter")));
		...
		RPC.execute(request, new AsyncCallback<DistributionsResponse>() {
			...
			public void onSuccess(DistributionsResponse result) {
				...
				if (result.hasTables())
					for (TableInterface table: result.getTables()) {
						...
						if (iConfig.isCanAdd()) {
							...
							header.getButton("add-distribution").setTitle(COURSE.titleAddDatePatternPreference(COURSE.accessAddDistributionPreference()));					
						}
						if (table.hasAnchor()) {
				...
				if (iConfig.isCanAdd()) {
					...
					footer.getButton("add-distribution").setTitle(COURSE.titleAddDatePatternPreference(COURSE.accessAddDistributionPreference()));
