...
public class ClassAssignmentsPage extends Composite {
	...
	private ClassAssignmentsFilterResponse iConfig;
	...
	public ClassAssignmentsPage() {
		...
		iFilter.getHeader().addCollapsibleHandler(new ValueChangeHandler<Boolean>() {
			...
			public void onValueChange(ValueChangeEvent<Boolean> event) {
				ToolBox.setSessionCookie("ClassAssignments.Filter", event.getValue() ? "1" : "0");
			}
		});
		...
		iFilter.getFooter().addButton("search", COURSE.actionSearchClassAssignments(), new ClickHandler() {
			@Override
	...
	protected void init() {
		RPC.execute(new ClassAssignmentsFilterRequest(), new AsyncCallback<ClassAssignmentsFilterResponse>() {
			@Override
			...
			@Override
			public void onSuccess(ClassAssignmentsFilterResponse result) {
				iConfig = result;
				...
				iFilter.getFooter().setEnabled("exportPdf", iConfig.isCanExportPdf());
	...
	protected void search(final AsyncCallback<Boolean> callback) {
		final ClassAssignmentsRequest request = new ClassAssignmentsRequest();
