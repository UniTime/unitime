...
public class SuggestionsBox extends UniTimeDialogBox {
	...
	public SuggestionsBox(TimeGrid.ColorProvider color, SpecialRegistrationContext specReg) {
		...
		if (iSpecReg != null) {
			...
			iAllChoices.addValueChangeHandler(new ValueChangeHandler<Boolean>() {
				...
				public void onValueChange(ValueChangeEvent<Boolean> event) {
					if (!iSpecReg.isDisclaimerAccepted() && event.getValue()) {
						iAllChoices.setValue(false);
						UniTimeConfirmationDialog.confirm(iSpecReg.areSpaceConflictsAllowed() ? (iSpecReg.areTimeConflictsAllowed() ? MESSAGES.disclaimerSpecRegAllowForTimeSpaceConflicts() : MESSAGES.disclaimerSpecRegAllowForSpaceConflicts()) : MESSAGES.disclaimerSpecRegAllowForTimeConflicts(), new Command() {
							@Override
							public void execute() {
		...
		iCallback = new AsyncCallback<Collection<ClassAssignmentInterface>>() {
			...
			public void showResults(Collection<ClassAssignmentInterface> result) {
	...
	public void open(final CourseRequestInterface request, final ArrayList<ClassAssignmentInterface.ClassAssignment> rows, RequestedCourse course, boolean useGwtConfirmations, final boolean waitList, AsyncCallback<ClassAssignmentInterface> callback) {
		...
		if (iAllChoices != null) {
			iAllChoices.setVisible(iSpecReg.isEnabled() && (iSpecReg.areTimeConflictsAllowed() || iSpecReg.areSpaceConflictsAllowed() || iSpecReg.areLinkedConflictsAllowed()));
			iAllChoices.setValue(false);
		}
