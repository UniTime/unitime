...
public class StudentStatusDialog extends UniTimeDialogBox{
	...
	protected void statusChanged() {
		...
		if (status != null) {
			...
			if (status.isEmail() && status.hasNotifications())
				iForm.addRow(MESSAGES.propEnabledStudentNotifications(), new Label(status.getNotifications()));
			...
			iForm.addRow(MESSAGES.propStatusReSchedule(), new WebTable.IconCell(status.isReSchedule() ? RESOURCES.on() : RESOURCES.off(),
					status.isReSchedule() ? MESSAGES.messageStatusReSchedule() : MESSAGES.messageStatusNoReSchedule(), null).getWidget());
	...
	public String getStatusLabel() {
		StudentStatusInfo status = getStudentStatusInfo(iStatus.getSelectedValue());
		if (status != null)
