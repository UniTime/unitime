 ...
 * @author Tomas Muller
 */
public class SolutionChangesPage extends Composite {
	...
	private SolutionChangesResponse iLastResponse;
	private DataTable iTable;
	private PreferenceLegend iLegend;

	public SolutionChangesPage() {
		iFilter = new PageFilter();
		iFilter.getHeader().setCollapsible(SolverCookie.getInstance().isSolutionChangesFilter());
		iFilter.getHeader().addCollapsibleHandler(new ValueChangeHandler<Boolean>() {
			@Override
			public void onValueChange(ValueChangeEvent<Boolean> event) {
				SolverCookie.getInstance().setSolutionChangesFilter(event.getValue());
			}
		});
		...
		iRootPanel.addStyleName("unitime-SolutionChangesPage");
	...
	protected void init() {
		RPC.execute(new SolutionChangesFilterRequest(), new AsyncCallback<SolutionChangesFilterResponse>() {
			@Override
			...
			@Override
			public void onSuccess(SolutionChangesFilterResponse result) {
	...
	public static native void createTriggers()/*-{
		$wnd.refreshPage = function() {
			@org.unitime.timetable.gwt.client.solver.SolutionChangesPage::__search()();
		};
	}-*/;
	...
	protected void search(final AsyncCallback<Boolean> callback) {
		final SolutionChangesRequest request = new SolutionChangesRequest();
		...
		RPC.execute(request, new AsyncCallback<SolutionChangesResponse>() {
			@Override
			public void onFailure(Throwable caught) {
				LoadingWidget.hideLoading();
				iFilter.getFooter().setErrorMessage(MESSAGES.failedToLoadSolutionChanges(caught.getMessage()));
				UniTimeNotifications.error(MESSAGES.failedToLoadSolutionChanges(caught.getMessage()), caught);
				iFilter.getFooter().setEnabled("search", true);
				if (callback != null)
			...
			@Override
			public void onSuccess(SolutionChangesResponse result) {
				LoadingWidget.hideLoading();
				iFilter.getFooter().clearMessage();
	...
	private void exportData(String format) {
		String query = "output=solution-changes." + format + iFilter.getQuery() + "&sort=" + SolverCookie.getInstance().getSolutionChangesSort();
	...
	protected void populate(FilterInterface filter, SolutionChangesResponse response) {
		iLastFilter = filter;
		...
		if (response.getRows().isEmpty()) {
			if (!response.hasMessage())
				iFilter.getFooter().setMessage(MESSAGES.errorSolutionChangesNoDataReturned());
			return;
		}
		
		UniTimeHeaderPanel header = new UniTimeHeaderPanel(MESSAGES.sectSolutionChanges());
		...
		if (iTable == null) {
			...
			iTable.addValueChangeHandler(new ValueChangeHandler<Integer>() {
				...
				public void onValueChange(ValueChangeEvent<Integer> event) {
					SolverCookie.getInstance().setSolutionChangesSort(event.getValue() == null ? 0 : event.getValue().intValue());
				}
			});
		} else {
			iTable.populate(response);
		}
		iTable.setValue(SolverCookie.getInstance().getSolutionChangesSort());
