...
public class ExamsPage extends Composite {
	...
	private ExamsFilterResponse iConfig;
	...
	public ExamsPage() {
		...
		iFilter.getFooter().addButton("add-exam", EXAM.buttonAddExamination(), new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				ToolBox.open(GWT.getHostPageBaseURL() + "examEdit.action");
			}
		});
		iFilter.getFooter().setEnabled("add-exam", false);
	
		iRootPanel = new SimplePanel(iPanel);
		iRootPanel.addStyleName("unitime-ExamsPage");
	...
	protected void init() {
		RPC.execute(new ExamsFilterRequest(), new AsyncCallback<ExamsFilterResponse>() {
			@Override
			...
			@Override
			public void onSuccess(ExamsFilterResponse result) {
				iConfig = result;
				...
				iFilter.getFooter().setEnabled("add-exam", iConfig.isCanAdd());
	...
	protected void search(final AsyncCallback<Boolean> callback) {
		final ExamsRequest request = new ExamsRequest();
	...
	protected void export(String format) {
		String sort = ToolBox.getSessionCookie("Exams.Sort");
