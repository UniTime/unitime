...
import org.unitime.timetable.gwt.client.exams.ExamsInterface.NotAssignedExamsFilterRequest;
import org.unitime.timetable.gwt.client.exams.ExamsInterface.NotAssignedExamsRequest;
...
public class NotAssignedExamsPage extends Composite {
	...
	public NotAssignedExamsPage() {
		...
		iFilter.getFooter().addButton("exportPdf", EXAM.buttonExportPDF(), new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				export("not-assigned-exams.pdf");
			}
		});
		iFilter.getFooter().setEnabled("exportPdf", false);
	
		iRootPanel = new SimplePanel(iPanel);
		iRootPanel.addStyleName("unitime-AssignedExamsPage");
	...
	protected void init() {
		RPC.execute(new NotAssignedExamsFilterRequest(), new AsyncCallback<ClassesFilterResponse>() {
			@Override
	...
	protected void search(final AsyncCallback<Boolean> callback) {
		final NotAssignedExamsRequest request = new NotAssignedExamsRequest();
	...
	protected void export(String format) {
		String sort = ToolBox.getSessionCookie("NotAssignedExams.Sort");
	...
	public static native void createTriggers()/*-{
		$wnd.refreshPage = function() {
			@org.unitime.timetable.gwt.client.exams.NotAssignedExamsPage::__search()();
		};
	}-*/;
	...
	public static void __search() {
		...
		NotAssignedExamsPage page = (NotAssignedExamsPage)RootPanel.get("UniTimeGWT:Body").getWidget(0);
