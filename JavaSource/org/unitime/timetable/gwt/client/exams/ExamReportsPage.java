...
public class ExamReportsPage extends Composite {
	...
	protected void init() {
		RPC.execute(new ExamReportsFilterRequest(), new AsyncCallback<ClassesFilterResponse>() {
			@Override
	...
	protected void search(final AsyncCallback<Boolean> callback) {
		final ExamReportsRequest request = new ExamReportsRequest();
	...
	protected void export(String format) {
		String sort = ToolBox.getSessionCookie(iConfig.getParameterValue("report", "ExamReport") + ".Sort");
