...
public class CurriculaServlet implements CurriculaService {
			...
			try {
				...
				if (c.isMultipleMajors() && !c.getMajors().isEmpty()) {
					for (Curriculum x: (List<Curriculum>)hibSession.createQuery(
							...
							Curriculum.class)
							.setParameter("curriculumId", c.getUniqueId()).setCacheable(true).list()) {
			...
			try {
				...
				if (!majors.isEmpty())
					curricula.addAll(hibSession.createQuery(
							...
							.setParameter("acadAreaId", acadAreaId).setParameterList("majorIds", majors).setCacheable(true).list());
	...
	private TreeSet<CurriculumInterface> loadCurriculaForACourse(org.hibernate.Session hibSession, TreeSet<AcademicClassificationInterface> academicClassifications, TreeSet<AcademicAreaInterface> academicAreas, CourseOffering courseOffering) throws CurriculaException, PageAccessException {
		...
		for (CurriculumCourse course : hibSession.createQuery(
				...
				.setParameter("courseId", courseOffering.getUniqueId()).setCacheable(true).list()) {
			...
			if (curriculum.getMajors().isEmpty()) {
				children = hibSession.createQuery("select c from Curriculum c where c.academicArea.uniqueId = :areaId and c.multipleMajors = true and " +
						...
						Curriculum.class
						).setParameter("areaId", curriculum.getAcademicArea().getUniqueId()).setParameter("courseId", course.getCourse().getUniqueId()).setCacheable(true).list();
				...
				children = hibSession.createQuery("select c from Curriculum c inner join c.majors m where c.academicArea.uniqueId = :areaId and m.uniqueId = :majorId and c.multipleMajors = true and " +
						"(select count(x) from CurriculumCourse x where x.course.uniqueId = :courseId and x.classification.curriculum.uniqueId = c.uniqueId) = 0",
						Curriculum.class
						).setParameter("areaId", curriculum.getAcademicArea().getUniqueId()).setParameter("majorId", curriculum.getMajors().iterator().next().getUniqueId())
						.setParameter("courseId", course.getCourse().getUniqueId()).setCacheable(true).list();
			}
	...
	public TreeSet<MajorInterface> loadMajors(Long curriculumId, Long academicAreaId, boolean multipleMajors) throws CurriculaException, PageAccessException {
		try {
			...
			try {
				...
				if (!multipleMajors) {
					majors.removeAll(
							hibSession.createQuery("select m from Curriculum c inner join c.majors m where c.academicArea.uniqueId = :academicAreaId and c.uniqueId != :curriculumId and c.multipleMajors = false",
									PosMajor.class)
							.setParameter("academicAreaId", academicAreaId).setParameter("curriculumId", (curriculumId == null ? -1l : curriculumId)).setCacheable(true).list());
				}
				for (PosMajor m: majors) {
					MajorInterface mi = new MajorInterface();
		...
		try {
			...
			try {
				...
				for (CourseOffering c: hibSession.createQuery(
						...
						.setParameter("course", course.toLowerCase())
						.setParameter("sessionId", sessionId)
	...
	public Boolean makeupCurriculaFromLastLikeDemands(boolean lastLike) throws CurriculaException, PageAccessException {
		...
		try {
			...
			try {
				...
				for (Curriculum c: hibSession.createQuery("from Curriculum where department.session.uniqueId = :sessionId", Curriculum.class).setParameter("sessionId", sessionId).list()) {
					ChangeLog.addChange(hibSession,
							getSessionContext(),
							c,
							c.getAbbv(),
			...
			try {
				...
				for (Iterator<CourseOffering> i = offering.getCourseOfferings().iterator(); i.hasNext(); ) {
					...
					if (area2major2clasf2ll != null) {
						List<AcademicClassification> classifications = hibSession.createQuery(
								"select c from AcademicClassification c where c.session.uniqueId = :sessionId", AcademicClassification.class)
								.setParameter("sessionId", offering.getSessionId()).setCacheable(true).list();
		...
		if (c.getMajors().isEmpty()) {
			...
			if (!c.isMultipleMajors())
				lines = hibSession.createQuery("select " + select + " from " + from + " where " + where + " group by " + group, Object[].class)
					.setParameter("sessionId", c.getAcademicArea().getSessionId()).setParameter("areaId", c.getAcademicArea().getUniqueId())
					.setCacheable(true).list();
		...
		if (majors.isEmpty()) {
			...
			if (!multipleMajors)
				lines = hibSession.createQuery("select " + select + " from " + from + " where " + where, Object[].class)
					.setParameter("sessionId", getAcademicSessionId()).setParameter("areaId", acadAreaId)
					.setCacheable(true).list();
	...
	private Hashtable<String, Hashtable<String, Integer>> loadClasfMajor2ll(org.hibernate.Session hibSession, Curriculum c) {
		...
		if (c.getMajors().isEmpty()) {
			...
			if (!c.isMultipleMajors()) {
				...
				group = "f.code";
				lines = hibSession.createQuery("select " + select + " from " + from + " where " + where + " group by " + group, Object[].class)
						.setParameter("sessionId", c.getAcademicArea().getSessionId()).setParameter("acadAbbv", c.getAcademicArea().getAcademicAreaAbbreviation())
						.setCacheable(true).list();
			}
		} else if (!c.isMultipleMajors() || c.getMajors().size() == 1) {
			// students with one major
	...
	private Hashtable<String, HashMap<String, Map<Long, Double>>> loadClasfMajor2ll(org.hibernate.Session hibSession, String acadAreaAbbv, Collection<PosMajor> majors, boolean multipleMajors) {
		...
		if (majors.isEmpty()) {
			...
			if (!multipleMajors) {
				...
				lines = hibSession.createQuery("select " + select + " from " + from + " where " + where, Object[].class)
						.setParameter("sessionId", getAcademicSessionId()).setParameter("acadAbbv", acadAreaAbbv)
						.setCacheable(true).list();
			}
		} else if (!multipleMajors || majors.size() == 1) {
	...
	private Hashtable<Long, Hashtable<Long, Integer>> loadClasfCourse2enrl(org.hibernate.Session hibSession, Curriculum c) {
		...
		String where = "c.uniqueId in (select e.courseOffering.uniqueId from StudentClassEnrollment e where e.student.uniqueId = s.uniqueId) and s.session.uniqueId = :sessionId and a.academicArea.uniqueId = :areaId";
	...
	private Map<Long, Map<Long, Map<Long, Map<Long, Double>>>> loadAreaMajorClasf2enrl(org.hibernate.Session hibSession, Long courseOfferingId) {
		...
		for (Object[] o : hibSession.createQuery(
				...
				.setParameter("courseId", courseOfferingId)
				.setCacheable(true).list()) {
	...
	private boolean hasSnapshotData(org.hibernate.Session hibSession, Long sessionId) {
		Long cnt = hibSession
				...
				.setParameter("sessId", sessionId).setCacheable(true).uniqueResult();
