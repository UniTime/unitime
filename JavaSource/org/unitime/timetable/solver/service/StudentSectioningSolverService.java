...
public class StudentSectioningSolverService implements SolverService<StudentSolverProxy> {
	...
	public DataProperties createConfig(Long settingsId, Map<Long, String> options) {
		...
		for (SolverParameterDef def: SolverPredefinedSettingDAO.getInstance().getSession().createQuery(
				"from SolverParameterDef where group.type = :type", SolverParameterDef.class).setParameter("type", SolverParameterGroup.SolverType.STUDENT.ordinal(), org.hibernate.type.IntegerType.INSTANCE).list()) {
