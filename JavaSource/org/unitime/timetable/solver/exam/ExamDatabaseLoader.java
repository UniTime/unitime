...
public class ExamDatabaseLoader extends ProblemLoader<Exam, ExamPlacement, ExamModel> {
    ...
    protected void loadStudents() {
        loadStudents(
                ...
                "o.ownerId=c.uniqueId", Object[].class).setParameter("sessionId", iSessionId, Long.class).setParameter("examTypeId", iExamTypeId, Long.class).list(),
                "class");
        loadStudents(
                ExamDAO.getInstance().getSession().createQuery(
                "select x.uniqueId, o.uniqueId, e.student.uniqueId from "+
                ...
                "o.ownerId=ioc.uniqueId", Object[].class).setParameter("sessionId", iSessionId, Long.class).setParameter("examTypeId", iExamTypeId, Long.class).list(),
                "config");
        loadStudents(
                ExamDAO.getInstance().getSession().createQuery(
                "select x.uniqueId, o.uniqueId, e.student.uniqueId from "+
                ...
                "o.ownerId=co.uniqueId", Object[].class).setParameter("sessionId", iSessionId, Long.class).setParameter("examTypeId", iExamTypeId, Long.class).list(),
                "course");
        loadStudents(
                ExamDAO.getInstance().getSession().createQuery(
                "select x.uniqueId, o.uniqueId, e.student.uniqueId from "+
                ...
                "o.ownerId=io.uniqueId", Object[].class).setParameter("sessionId", iSessionId, Long.class).setParameter("examTypeId", iExamTypeId, Long.class).list(),
                "offering");
    }
                ...
                if (period.overlap(a)) {
                    ...
                    if (studentIds==null)
                        studentIds = ExamDAO.getInstance().getSession().createQuery(
                                ...
                                setParameter("classId", a.getClassId(), Long.class).list(); 
    ...
    */
    ...
    protected void loadAvailabilitiesFromEvents() {
        ...
        for (Iterator i=overlappingCourseEvents.iterator();i.hasNext();) {
            ...
            iProgress.debug("Course-related event "+meeting.getEvent().getEventName()+"/"+meeting.getTimeLabel()+" overlaps with period "+period+" ("+studentIds.size()+" students, "+instructorIds.size()+" instructors)");
            
            unavailabilitiesThisEvent.put(period, unavailability);
            getModel().addUnavailability(unavailability);
        }
        
        for (Iterator i=overlappingExamEvents.iterator();i.hasNext();) {
            iProgress.incProgress();
            Object[] o = (Object[])i.next();
            Long eventId = (Long)o[0];
            ExamPeriod period = iPeriods.get((Long)o[1]);
            ...
            Set<ExamInstructor> instructorsThisEvent = instructors.get(eventId);
            if (instructorsThisEvent == null) {
            	instructorsThisEvent = new HashSet<ExamInstructor>();
            	ExamEvent event = ExamEventDAO.getInstance().get(meeting.getEvent().getUniqueId());
            	for (DepartmentalInstructor deptInst: event.getInstructors()) {
            		ExamInstructor instructor = getInstructor(deptInst);
                    if (instructor!=null)
                    	instructorsThisEvent.add(instructor);
            	}
            ...
            iProgress.debug("Examination event "+meeting.getEvent().getEventName()+"/"+meeting.getTimeLabel()+" overlaps with period "+period+" ("+studentIds.size()+" students, "+instructorIds.size()+" instructors)");
    ...
    protected void loadDistributions() {
        List<DistributionPref> distPrefs = DistributionPrefDAO.getInstance().getSession().createQuery(
                ...
                setParameter("sessionId", iSessionId, Long.class).
                setParameter("examTypeId", iExamTypeId, Long.class).list();
