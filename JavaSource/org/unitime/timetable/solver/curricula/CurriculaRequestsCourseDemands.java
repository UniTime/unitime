 ...
 * @author Tomas Muller
 */
public class CurriculaRequestsCourseDemands implements StudentCourseDemands, NeedsStudentIdGenerator {
		...
		if (cc.getCurriculum().getMajors().isEmpty()) {
			...
			if (!cc.getCurriculum().isMultipleMajors())
				lines = hibSession.createQuery("select " + select + " from " + from + " where " + where, Object[].class)
					.setParameter("sessionId", cc.getCurriculum().getAcademicArea().getSessionId())
					.setParameter("acadAreaId", cc.getCurriculum().getAcademicArea().getUniqueId())
					.setParameter("clasfId", cc.getAcademicClassification().getUniqueId())
			...
			for (PosMajor major: cc.getCurriculum().getMajors()) {
				if (idx == 0) {
					...
					where += " and a" + idx + ".academicArea.uniqueId = :acadAreaId and a" + idx + ".academicClassification.uniqueId = :clasfId and a" + idx + ".major.uniqueId = :m" + idx;
				}
				...
				if (iLoadedCurricula == null || iLoadedCurricula.isEmpty()) {
					...
					other: for (WeightedStudentId student: other) {
						...
						for (AreaClasfMajor acm: student.getMajors()) {
							Set<String> majors = iLoadedCurricula.get(acm.getArea());
