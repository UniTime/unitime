    ...
    public String getHistoryTable(boolean simple, HttpServletRequest request, String name, Vector history) {
        ...
        try {
        	...
        	for (Enumeration e=history.elements();e.hasMoreElements();idx++) {
        	    ...
        	    if (simple)
            	    webTable.addLine((hasBefore?"onClick=\"window.open('suggestions.do?hist="+idx+"&op=ShowHistory','suggestions','width=1000,height=600,resizable=yes,scrollbars=yes,toolbar=no,location=no,directories=no,status=yes,menubar=no,copyhistory=no');\"":null),
            	    		new String[] {
