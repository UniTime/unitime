    ...
    public String getChangesTable(boolean simple, boolean reversed, HttpServletRequest request, String name, Vector changes) {
        ...
        try {
        	...
        	for (Enumeration e=changes.elements();e.hasMoreElements();idx++) {
        	    ...
        	    if (simple)
            	    webTable.addLine((link!=null?"onClick=\"window.open('suggestions.do?"+link+"&op=Try','suggestions','width=1000,height=600,resizable=yes,scrollbars=yes,toolbar=no,location=no,directories=no,status=yes,menubar=no,copyhistory=no');\"":null),
            	    		new String[] {
            	    			className,
            	    			dates,
            	    			time,
