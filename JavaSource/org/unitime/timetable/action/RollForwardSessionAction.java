		...
		protected void execute() throws Exception {
        	...
        	if (iErrors.isEmpty() && iForm.getRollForwardDepartments()) {
    			...
    			try {
    				...
    				if (iForm.validateDepartmentRollForward(toAcadSession, iErrors))
    					sessionRollForward.rollDepartmentsForward(iErrors, iForm, copyBetweenSessionHelper);	
    		        tx.commit();
    			} catch (Exception e) {
    				tx.rollback();
        	...
        	if (iErrors.isEmpty() && iForm.getRollForwardSessionConfig()) {
    			...
    			try {
    				...
    				sessionRollForward.rollSessionConfigurationForward(iErrors, iForm, copyBetweenSessionHelper);
    				tx.commit();
    			} catch (Exception e) {
    				tx.rollback();
        	...
        	if (iErrors.isEmpty() && iForm.getRollForwardManagers()) {
    			...
    			try {
    				...
    				if (iForm.validateManagerRollForward(toAcadSession, iErrors))
    					sessionRollForward.rollManagersForward(iErrors, iForm, copyBetweenSessionHelper);
    				tx.commit();
    			} catch (Exception e) {
    				tx.rollback();
        	...
        	if (iErrors.isEmpty() && iForm.getRollForwardRoomData()) {
    			...
    			try {
    				...
    				if (iForm.validateBuildingAndRoomRollForward(toAcadSession, iErrors))
    					sessionRollForward.rollBuildingAndRoomDataForward(iErrors, iForm, copyBetweenSessionHelper);
    				tx.commit();
    			} catch (Exception e) {
    				tx.rollback();
