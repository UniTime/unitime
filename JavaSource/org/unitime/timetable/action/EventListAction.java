...
public class EventListAction extends Action {
	...
	public ActionForward execute(
        ...
        if ("Export PDF".equals(op)) {
            File pdfFile = new PdfEventTableBuilder().pdfTableForEvents(sessionContext, myForm);
            if (pdfFile!=null) request.setAttribute(Constants.REQUEST_OPEN_URL, "temp/"+pdfFile.getName());
        }
        
        if ("Export CSV".equals(op)) {
            File csvFile = new CsvEventTableBuilder().csvTableForEvents(sessionContext, myForm);
            if (csvFile!=null) request.setAttribute(Constants.REQUEST_OPEN_URL, "temp/"+csvFile.getName());
        }

        if ("iCalendar".equals(op)) {
        	String url = new CalendarEventTableBuilder().calendarUrlForEvents(sessionContext, myForm);
