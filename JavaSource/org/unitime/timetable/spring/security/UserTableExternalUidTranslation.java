...
public class UserTableExternalUidTranslation implements ExternalUidTranslation {
    ...
    public String uid2ext(String username) {
		...
		try {
			User user =  hibSession.createQuery("from User where username=:username", User.class).setParameter("username", username, org.hibernate.type.StringType.INSTANCE).setMaxResults(1).uniqueResult();
    ...
    public String ext2uid(String externalUniqueId) {
		org.hibernate.Session hibSession = UserDAO.getInstance().createNewSession();
		try {
			User user =  hibSession.createQuery("from User where externalUniqueId=:externalUniqueId", User.class).setParameter("externalUniqueId", externalUniqueId, org.hibernate.type.StringType.INSTANCE).setMaxResults(1).uniqueResult();
