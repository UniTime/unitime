 ...
 * @author Tomas Muller
 */
@GwtRpcImplements(ExamConflictStatisticsFilterRequest.class)
public class ExamConflictStatisticsFilterBackend implements GwtRpcImplementation<ExamConflictStatisticsFilterRequest, ConflictStatisticsFilterResponse> {
	...
	@Override
	public ConflictStatisticsFilterResponse execute(ExamConflictStatisticsFilterRequest request, SessionContext context) {
		...
		mode.setDefaultValue(context.getUser().getProperty("Ecbs.type", "0"));
		...
		limit.setDefaultValue(context.getUser().getProperty("Ecbs.limit", "25.0"));
		...
		properties.setSolver(examinationSolverService.getSolver() != null);
		response.setSuggestionProperties(properties);
		properties.setFirstDay(ApplicationProperty.TimePatternFirstDayOfWeek.intValue());
		
		SolverPageBackend.fillSolverWarnings(context, examinationSolverService.getSolver(), SolverType.EXAM, response);
