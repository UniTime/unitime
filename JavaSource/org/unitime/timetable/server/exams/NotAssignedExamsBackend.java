...
@GwtRpcImplements(NotAssignedExamsRequest.class)
public class NotAssignedExamsBackend implements GwtRpcImplementation<NotAssignedExamsRequest, OfferingsResponse> {
	...
	@Override
	public OfferingsResponse execute(NotAssignedExamsRequest request, SessionContext context) {
		context.checkPermission(Right.NotAssignedExaminations);
		...
		BackTracker.markForBack(
				context, 
				"notAssignedExams?examType=" + type.getUniqueId() + "&subjectArea=" + request.getFilter().getParameterValue("subjectArea"),
				MESSAGES.backNotAssignedExaminations(), 
				true, true);
		
		NotAssignedExamsTableBuilder builder = new NotAssignedExamsTableBuilder(context, request.getBackType(), request.getBackId());
		
		response.add(builder.generateNotAssignedExamsTable(type, examinationSolverService.getSolver(), request.getFilter(), subjectArea.split(",")));
