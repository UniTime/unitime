...
import org.unitime.timetable.gwt.client.exams.ExamsInterface.ExamReportsRequest;
...
@GwtRpcImplements(ExamReportsRequest.class)
public class ExamReportsBackend implements GwtRpcImplementation<ExamReportsRequest, OfferingsResponse> {
	...
	@Override
	public OfferingsResponse execute(ExamReportsRequest request, SessionContext context) {
		...
		String report = request.getFilter().getParameterValue("report");
		if (report == null || report.isEmpty() || ExamReport.valueOf(report) == null)
			throw new GwtRpcException(MESSAGES.errorNoReportSelected());
		...
		BackTracker.markForBack(
				context, 
				"examAssignmentReport?examType=" + type.getUniqueId() + "&subjectArea=" + request.getFilter().getParameterValue("subjectArea"),
				ExamReport.valueOf(report).getLabel(), 
				true, true);
		
		ExamReportsTableBuilder builder = new ExamReportsTableBuilder(context, request.getBackType(), request.getBackId());
		
		response.add(builder.generateExamReportTable(type, examinationSolverService.getSolver(), request.getFilter(), subjectArea.split(",")));
		
		switch (ExamReport.valueOf(report)) {
