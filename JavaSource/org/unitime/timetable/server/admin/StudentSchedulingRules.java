...
public class StudentSchedulingRules implements AdminTable {
	...
	public SimpleEditInterface load(SessionContext context, org.hibernate.Session hibSession) {
		...
		for (StudentSchedulingRule rule: (List<StudentSchedulingRule>)StudentSchedulingRuleDAO.getInstance().getSession().createQuery(
				"from StudentSchedulingRule order by ord").list()) {
