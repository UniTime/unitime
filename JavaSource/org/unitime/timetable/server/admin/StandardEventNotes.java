...
public class StandardEventNotes implements AdminTable {
	...
	public SimpleEditInterface load(SessionContext context, Session hibSession) {
		...
		for (StandardEventNote note: hibSession.createQuery("from StandardEventNoteGlobal order by reference", StandardEventNote.class).setCacheable(true).list()) {
		...
		for (StandardEventNote note: hibSession.createQuery(
				"from StandardEventNoteSession where session.uniqueId = :sessionId order by reference", StandardEventNote.class).setParameter("sessionId", sessionId).setCacheable(true).list()) {
			Record r = data.addRecord(note.getUniqueId());
		...
		for (Department department: Department.getUserDepartments(context.getUser())) {
			...
			for (StandardEventNote note: hibSession.createQuery(
					"from StandardEventNoteDepartment where department.uniqueId = :deptId order by reference", StandardEventNote.class).setParameter("deptId", department.getUniqueId()).setCacheable(true).list()) {
				Record r = data.addRecord(note.getUniqueId());
	...
	public void save(SimpleEditInterface data, SessionContext context, Session hibSession) {
		...
		if (context.hasPermission(Right.StandardEventNotesGlobalEdit))
			for (StandardEventNote note: hibSession.createQuery("from StandardEventNoteGlobal", StandardEventNote.class).setCacheable(true).list()) {
		...
		if (context.hasPermission(sessionId, "Session", Right.StandardEventNotesSessionEdit))
			for (StandardEventNote note: hibSession.createQuery("from StandardEventNoteSession where session.uniqueId = :sessionId", StandardEventNote.class).setParameter("sessionId", sessionId).setCacheable(true).list()) {
				Record r = data.getRecord(note.getUniqueId());
				if (r == null)
					delete(note, context, hibSession);
				else
		...
		for (Department department: Department.getUserDepartments(context.getUser())) {
			...
			for (StandardEventNote note: hibSession.createQuery("from StandardEventNoteDepartment where department.uniqueId = :deptId", StandardEventNote.class).setParameter("deptId", department.getUniqueId()).setCacheable(true).list()) {
				Record r = data.getRecord(note.getUniqueId());
				if (r == null)
					delete(note, context, hibSession);
				else
