	...
	public boolean isUsed(Long sessionId) {
		if (sessionId == null) {
			return (ExamTypeDAO.getInstance().getSession().createQuery(
					...
					.setParameter("typeId", getUniqueId()).setCacheable(true).uniqueResult()).longValue() > 0;
		} else {
			return (ExamTypeDAO.getInstance().getSession().createQuery(
					"select count(p) from ExamPeriod p where p.examType.uniqueId = :typeId and p.session.uniqueId = :sessionId", Number.class)
					.setParameter("typeId", getUniqueId()).setParameter("sessionId", sessionId).setCacheable(true).uniqueResult()).longValue() > 0;
		}
	}
