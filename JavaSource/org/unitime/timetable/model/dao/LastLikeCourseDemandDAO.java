...
public class LastLikeCourseDemandDAO extends _RootDAO<LastLikeCourseDemand,Long> {
	...
	public List<LastLikeCourseDemand> findByStudent(org.hibernate.Session hibSession, Long studentId) {
		return hibSession.createQuery("from LastLikeCourseDemand x where x.student.uniqueId = :studentId", LastLikeCourseDemand.class).setParameter("studentId", studentId).list();
	}

	@SuppressWarnings("unchecked")
	public List<LastLikeCourseDemand> findBySubjectArea(org.hibernate.Session hibSession, Long subjectAreaId) {
		return hibSession.createQuery("from LastLikeCourseDemand x where x.subjectArea.uniqueId = :subjectAreaId", LastLikeCourseDemand.class).setParameter("subjectAreaId", subjectAreaId).list();
	}
}
