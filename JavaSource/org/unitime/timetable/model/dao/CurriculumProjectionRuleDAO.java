...
public class CurriculumProjectionRuleDAO extends _RootDAO<CurriculumProjectionRule,Long> {
	...
	public List<CurriculumProjectionRule> findByAcademicArea(org.hibernate.Session hibSession, Long academicAreaId) {
		return hibSession.createQuery("from CurriculumProjectionRule x where x.academicArea.uniqueId = :academicAreaId", CurriculumProjectionRule.class).setParameter("academicAreaId", academicAreaId).list();
	}

	@SuppressWarnings("unchecked")
	public List<CurriculumProjectionRule> findByMajor(org.hibernate.Session hibSession, Long majorId) {
		return hibSession.createQuery("from CurriculumProjectionRule x where x.major.uniqueId = :majorId", CurriculumProjectionRule.class).setParameter("majorId", majorId).list();
	}
	...
	public List<CurriculumProjectionRule> findByAcademicClassification(org.hibernate.Session hibSession, Long academicClassificationId) {
		return hibSession.createQuery("from CurriculumProjectionRule x where x.academicClassification.uniqueId = :academicClassificationId", CurriculumProjectionRule.class).setParameter("academicClassificationId", academicClassificationId).list();
	}
}
