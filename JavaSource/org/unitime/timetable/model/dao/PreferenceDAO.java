...
public class PreferenceDAO extends _RootDAO<Preference,Long> {
	...
	public List<Preference> findByOwner(org.hibernate.Session hibSession, Long ownerId) {
		return hibSession.createQuery("from Preference x where x.owner.uniqueId = :ownerId", Preference.class).setParameter("ownerId", ownerId).list();
	}

	@SuppressWarnings("unchecked")
	public List<Preference> findByPrefLevel(org.hibernate.Session hibSession, Long prefLevelId) {
		return hibSession.createQuery("from Preference x where x.prefLevel.uniqueId = :prefLevelId", Preference.class).setParameter("prefLevelId", prefLevelId).list();
	}
}
