...
@Service("org.unitime.timetable.export.Exporter:exams-grid.pdf")
public class ExamGridPDF extends ClassesCSV {
	
	@Override
	public String reference() {
		return "exams-grid.pdf";
	}
	...
	public void export(ExportHelper helper) throws IOException {
		...
		for (int row = 0; row < export.getRowCount(); row++) {
			...
			for (int idx = 0; idx < rowspan; idx++) {
				...
				if (firstLine) { printer.setupHeader(line); firstLine = false; }
				printer.printLine(line);
			}
		}
		if (lastName != null) printer.flushTable(null);
