...
import org.unitime.timetable.server.exams.NotAssignedExamsTableBuilder;
import org.unitime.timetable.solver.exam.ExamSolverProxy;

@Service("org.unitime.timetable.export.Exporter:not-assigned-exams.csv")
public class NotAssignedExamsCSV extends ClassesCSV {
	
	@Override
	public String reference() {
		return "not-assigned-exams.csv";
	}
	
	@Override
	public void export(ExportHelper helper) throws IOException {
		helper.getSessionContext().checkPermissionAnySession(helper.getAcademicSessionId(), Right.NotAssignedExaminations);
		exportDataCsv(getNotAssignedExams(helper, examinationSolverService.getSolver()), helper);
	}
	
	protected static List<TableInterface> getNotAssignedExams(ExportHelper helper, ExamSolverProxy solver) {
    	List<TableInterface> response = new ArrayList<TableInterface>();
    	
    	NotAssignedExamsTableBuilder builder = new NotAssignedExamsTableBuilder(
		...
		response.add(sorted(builder.generateNotAssignedExamsTable(
				type,
				solver,
		        filter, 
