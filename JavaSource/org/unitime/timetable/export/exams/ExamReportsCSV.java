...
import org.unitime.timetable.server.exams.ExamReportsTableBuilder;
import org.unitime.timetable.server.exams.ExamReportsTableBuilder.ExamReport;
import org.unitime.timetable.solver.exam.ExamSolverProxy;

@Service("org.unitime.timetable.export.Exporter:exam-report.csv")
public class ExamReportsCSV extends ClassesCSV {
	
	@Override
	public String reference() {
		return "exam-report.csv";
	}
	
	@Override
	public void export(ExportHelper helper) throws IOException {
		helper.getSessionContext().checkPermissionAnySession(helper.getAcademicSessionId(), Right.ExaminationReports);
		exportDataCsv(getExamReport(helper, examinationSolverService.getSolver()), helper);
	}
	
	protected static List<TableInterface> getExamReport(ExportHelper helper, ExamSolverProxy solver) {
    	List<TableInterface> response = new ArrayList<TableInterface>();
    	
    	ExamReportsTableBuilder builder = new ExamReportsTableBuilder(
		...
		String report = filter.getParameterValue("report");
		if (report == null || report.isEmpty() || ExamReport.valueOf(report) == null)
			throw new GwtRpcException(EXAM.errorNoReportSelected());
		
		response.add(sorted(builder.generateExamReportTable(
				type,
				solver,
		        filter, 
