...
@Service("org.unitime.timetable.export.Exporter:exams-grid.csv")
public class ExamGridCSV extends ClassesCSV {
	
	@Override
	public String reference() {
		return "exams-grid.csv";
	}
	...
	public void export(ExportHelper helper) throws IOException {
		...
		for (int row = 0; row < export.getRowCount(); row++) {
			String[] line = new String[export.getCellCount(row)];
			for (int col = 0; col < export.getCellCount(row); col++)
				line[col] = export.getElement(row, col).toString();
			if (row == 0)
				printer.printHeader(line);
			else
