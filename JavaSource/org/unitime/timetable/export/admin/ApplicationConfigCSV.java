...
@Service("org.unitime.timetable.export.Exporter:application-config.csv")
public class ApplicationConfigCSV extends ClassesCSV {
	@Autowired private ApplicationContext applicationContext;

	@Override
	public String reference() {
		return "application-config.csv";
	}
	
	@Override
	public void export(ExportHelper helper) throws IOException {
		GwtRpcImplementation<ApplicationConfigRequest, ApplicationConfigResponse> service = (GwtRpcImplementation<ApplicationConfigRequest, ApplicationConfigResponse>)applicationContext.getBean(ApplicationConfigRequest.class.getName());
		ApplicationConfigRequest request = new ApplicationConfigRequest();
		request.setExport(true);
		ApplicationConfigResponse response = service.execute(request, helper.getSessionContext());
