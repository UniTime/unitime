...
public class PointInTimeDataExport extends BaseExport {
	...
	private TreeSet<InstructionalOffering> findOfferingsWithClasses(Session acadSession) {
		...
		querySb.append("select io, cco ")
		       ...
		       //.append(" left join fetch oc.responsibility as otr")
		       ...
		       //.append(" left join fetch ci.responsibility as ctr")
	...
	private void exportProgram(Program program) {
    	Element programElement = programsElement.addElement(sProgramElementName);
    	programElement.addAttribute(sUniqueIdAttribute, program.getUniqueId().toString());
    	programElement.addAttribute(sCodeAttribute, program.getReference());
