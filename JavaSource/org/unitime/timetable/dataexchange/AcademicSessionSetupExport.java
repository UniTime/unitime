...
public class AcademicSessionSetupExport extends BaseExport {
	...
	protected void exportSolverGroups(Element root, Session session) {
		...
		for (SolverGroup g: new TreeSet<SolverGroup>(getHibSession().createQuery(
				"from SolverGroup where session.uniqueId = :sessionId", SolverGroup.class).setParameter("sessionId", session.getUniqueId()).list())) {
	...
	protected void exportTimePatterns(Element root, Session session) {
		...
		for (TimePattern tp: new TreeSet<TimePattern>(getHibSession().createQuery(
				"from TimePattern where session.uniqueId = :sessionId", TimePattern.class).setParameter("sessionId", session.getUniqueId()).list())) {
