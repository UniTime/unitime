	...
	private Location findLocation(Long sessionId, Element element){
		if (element.attributeValue("id") != null) {
			Room room = getHibSession().createQuery(
					...
					.setParameter("externalId", element.attributeValue("id"))
					.setCacheable(true)
					.setMaxResults(1)
					.uniqueResult();
		...
		if (element.attributeValue("building") != null && element.attributeValue("roomNbr") != null) {
			Room room = getHibSession().createQuery(
					...
					.setParameter("sessionId", sessionId)
					.setParameter("building", element.attributeValue("building"))
					.setParameter("roomNbr", element.attributeValue("roomNbr"))
					.setCacheable(true)
					.setMaxResults(1)
					.uniqueResult();
			if (room != null) return room;
		...
		if (element.attributeValue("name") != null) {
			Room room = getHibSession().createQuery(
					...
					.setParameter("sessionId", sessionId)
					.setParameter("name", element.attributeValue("name"))
					.setCacheable(true)
					.setMaxResults(1)
					.uniqueResult();
			if (room != null) return room;
