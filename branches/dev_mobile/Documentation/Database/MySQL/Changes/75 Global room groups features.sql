...
from room_group g, sessions s, (select @rownum:=@id) r where g.global = 1 and not exists
...
alter table room_feature add session_id decimal(20,0);
...
from room_feature f, sessions s, (select @rownum:=@id) r
